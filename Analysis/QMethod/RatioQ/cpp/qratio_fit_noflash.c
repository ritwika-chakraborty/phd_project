...
TH1D *qHist_1D[25],*h[25],*h_sum0, *h_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno,*h_res, *hfftsq;
TH1 *hfft,*hfft2;
TH1F *hlm;
...
void qratio_fit_noflash()
 ...
 cout<<sum<<" "<<endl;
  ...
  h_sum0= new TH1D("calo histogram sum", "h_sum0", h[1]->GetNbinsX(), 100001, 352001);
  h_sum0->Sumw2(kTRUE);
  ...
  double binwidth=h_sum0->GetBinWidth(1);
   int nbins=h_sum0->GetNbinsX();
   double binlowedge=h_sum0->GetBinLowEdge(1);
   double binhighedge=h_sum0->GetBinLowEdge(nbins)+binwidth;

   cout<<binwidth<<" "<<nbins<<" "<<binlowedge<<" "<<binhighedge<<" "<<endl;

   h_sum0->SetBins(nbins, rawBinToNs*(binlowedge-inject_time), rawBinToNs*(binhighedge-inject_time));

   cout<<h_sum0->GetBinWidth(1)<<" "<<h_sum0->GetNbinsX()<<" "<<h_sum0->GetBinLowEdge(1)<<" "<<(h_sum0->GetBinLowEdge(h_sum0->GetNbinsX())+h_sum0->GetBinWidth(1))<<" "<<endl;

   //   h_sum->Draw();
   h_sum= new TH1D("calo histogram sum new", "h_sum", h_sum0->GetNbinsX()-h_sum0->FindBin(fit_start), h_sum0->GetBinLowEdge(h_sum0->FindBin(fit_start)+1), h_sum0->GetBinLowEdge(h_sum0->GetNbinsX())+h_sum0->GetBinWidth(1));
   int j=1;
   for(int i=h_sum0->FindBin(fit_start); i<=h_sum0->GetNbinsX();i++)
     {
       h_sum->SetBinContent(j,h_sum0->GetBinContent(i));
       h_sum->SetBinError(j, h_sum0->GetBinError(i));
       j=j+1;
     }
   // return;
   h_sum->Rebin(4);
   ...
   for(int ibin=1;ibin<=h_sum->GetNbinsX();ibin++)
     {
       hp->SetBinContent(ibin,h_sum->GetBinContent(ibin+29));
     }

   for(int ibin=29;ibin<=h_sum->GetNbinsX();ibin++)
     {
       hm->SetBinContent(ibin,h_sum->GetBinContent(ibin-29));
     }
   h1->Scale(0.25);
   h2->Scale(0.25);
   hp->Scale(0.25);
   hm->Scale(0.25);
	...
	//h_num->Draw();
	//  fit_func->Draw("same");
	 	h_num->Fit("fprec","R","",fit_start,fit_stop);
	...
	for (int ibin = ((fit_start + 1- h_num->GetBinLowEdge(1))/h_num->GetBinWidth(1))+1; ibin <= ((fit_stop + 1 - h_num->GetBinLowEdge(1))/h_num->GetBinWidth(1))+1; ++ibin)
     {
      ...
      double res =  (h_num->GetBinContent(ibin)- fit_func->Eval( h_num->GetBinCenter(ibin) ) );
  ...
  c2->cd(3);
   ...
   hfft->Draw();     
     */    
   /*   hfftsq=new TH1D("residual fft squared","residual fft squared",h_res->GetNbinsX(),0,h_res->GetNbinsX()/((h_res->GetBinLowEdge(h_res->GetNbinsX())+h_res->GetBinWidth(1))));

   for(int ibin=1;ibin<=h_res->GetNbinsX();ibin++)
     {
       hfftsq->SetBinContent(ibin, hfft->GetBinContent(ibin)*hfft->GetBinContent(ibin));
     }

   hfft2=hfftsq->FFT(hfft2,"MAG");
   hfft2->Draw();
   */
     h_ratio->Rebin(2);
     h_ratio->Scale(0.5);
   ...
   for(int ibin=1;ibin<=h_num->GetNbinsX();ibin++)
     ...
     fit_func2= new TF1("fprec2", fprec2,  30000,  309000, 3);
	fit_func2->SetParNames("A", "Blind R", "Phi");
     fit_func2->SetNpx(1000000);
     fit_func2->SetParameters(0.2, 0.0, 3.14/2);
	...
	for (int ibin = ((fit_start + 1- h_ratio->GetBinLowEdge(1))/h_ratio->GetBinWidth(1))+1; ibin <= ((fit_stop + 1 - h_ratio->GetBinLowEdge(1))/h_ratio->GetBinWidth(1))+1; ++ibin)
     {
      ...
      // if(h_ratio->GetBinError(ibin)!=0){res=(res/h_ratio->GetBinError(ibin));}
      h_res->SetBinContent(ibin, (res)  );
