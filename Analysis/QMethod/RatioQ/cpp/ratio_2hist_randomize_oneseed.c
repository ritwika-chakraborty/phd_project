...
Double_t fprec23(Double_t *x, Double_t *par)
{
    ...
    return (f*Ncbo*Nvw*Nvbo*Npr - ff*Ncbof*Nvwf*Nvbof*Nprf)/(f*Ncbo*Nvw*Nvbo*Npr + ff*Ncbof*Nvwf*Nvbof*Nprf);
...
*/
void ratio_2hist_randomize_oneseed()
{
 ...
 if(useerrorbars)
	...
	//	cout<<( (t1*t1) + (t2*t2) + (t3*t3) )<<endl;
       ...
       {
	 h_num->SetBinError(ibin,  sqrt( (t2*t2) + (t2*t2) ) );
       }
      }
 ...
 cout<<hsum3->GetBinWidth(500)<<endl;
