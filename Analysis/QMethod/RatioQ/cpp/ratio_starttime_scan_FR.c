...
TFile *_file[25];
TDirectory *dir[25];
TH1D *h[25],*qHist_1D[25],*h_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno, *hcalo, *hfit, *hr[50], *h_res[50], *hr_sum, *hr_noshift, *hr_shift;
TCanvas *c,*c1, *c2,*c3;
TGraphErrors *gr1;
TH1 *hfft[50];
...
void ratio_starttime_scan_FR()
{
  ...
  hlm->SetName("hlm");
     ...
     /*  fit_func7->SetParameter(0, 2.29082e-01);
      fit_func7->SetParameter(1, 0);
      fit_func7->SetParameter(2, 2.22448e+00);
      fit_func7->SetParameter(3, 2.46515e-03);
      fit_func7->SetParameter(4, 2.48933e+05);
      fit_func7->SetParameter(5, 2.34010e-03);
      fit_func7->SetParameter(6, 5.02505e-01);
     */
	...
	hr_sum=(TH1D*)hr_shift->Clone();
	hr_sum->Reset();

	for(int ibin=1;ibin<=hr_shift->GetNbinsX();ibin++)
	  {
	    hr_sum->SetBinContent(ibin,0.5*(hr_noshift->GetBinContent(ibin)+hr_shift->GetBinContent(ibin)));
	    hr_sum->SetBinError(ibin,0.5*(hr_noshift->GetBinError(ibin)+hr_shift->GetBinError(ibin)));
	  }
