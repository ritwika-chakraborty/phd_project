...
//char root_file_name[128] = "run3BtoM_thesh300_nofbfDQC_temp.root";
//char root_file_name[128] = "run2_thresh300_fbfDQC_wndw_8.root";
char root_file_name[128] = "run2H_NE.root";
//char root_file_name[128] = "r2h_w8.root";
char *histname = new char[10];
char *histname2 = new char[10];
TVirtualFitter *gFitter;
TFile *_file[25];
TDirectory *dir[25];
TH1D *h[25],*hrm1[25],*hrm2[25],*hrm3[25],*hrm4[25],*qHist_1D[25],*h_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno, *hcalo, *hfit, *hr[25], *h_res, *hr_sum, *hcov_1_p, *hcov_1_m,*hcov_13_p,*hcov_13_m,*hcov_14_p, *hcov_14_m, *hcov_15_p, *hcov_15_m, *hcov_27_p, *hcov_27_m,*hcov_28_p, *hcov_28_m, *hcov_29_p, *hcov_29_m, *hr_noshift, *hr_shift, *hcomp_sum;
   ...
   for (Int_t i = 0; i < (mdim)*(mdim); i++)
	...
	const Int_t ic = (i)%(mdim);
		...
		if(ir==ic-1) //diag+1
	  {
	    ...
	    // data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	     data[i]=scaling_factor*covar;
	    // data[i]=scaling_factor*0.165838*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	  }
       	 ...
       	 if(ir==ic-(nbinshift-1)) //diag+13
	  {
	    ...
	    //data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	     data[i]=scaling_factor*covar;
	    //data[i]=scaling_factor*-0.10952*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
       	  }
       	...
       	if(ir==ic-(nbinshift)) //diag+14
	  {
	    ...
	    //  data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	    data[i]=scaling_factor*covar;
	    // data[i]=scaling_factor*-0.657942*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
       	  }
		...
		if(ir==ic-(nbinshift+1)) //diag+15
	  {
	    ...
	    //data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	     data[i]=scaling_factor*covar;
	    // data[i]=scaling_factor*-0.109439*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
       	  }
		 ...
		 if(ir==ic-((2*nbinshift)-1)) //diag+27
	  {
	    ...
	    // data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	    data[i]=scaling_factor*covar;
	    // data[i]=scaling_factor*0.0274031*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
      	  }
	...
	if(ir==ic+((2*nbinshift)-1))//diag-27
	   	...
	   	if(ir==ic-(2*nbinshift))//diag+28
	  {
	    ...
	    // data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	    data[i]=scaling_factor*covar;
	    //data[i]=scaling_factor*0.163612*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
      	  }
	...
	if(ir==ic+(2*nbinshift))//diag-28
       	...
       	if(ir==ic-((2*nbinshift)+1))//diag+29
	  {
	    ...
	    //data[i]=covar/(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
	     data[i]=scaling_factor*covar;
	    // data[i]=scaling_factor*0.0273977*(hr_sum->GetBinError(ir)*hr_sum->GetBinError(ic));
      	  }
...
void ratio_4h_FR_cov_symm_construct_per_run()
{
  ...
  */
  ...
  //   cov.Print();
	...
	hcov = new TH2D("hcov", "cov_matrix_hist", 2100, 0.0, 2100.0, 2100, 0.0, 2100.0);
  ...
  */
   ...
   for(int irow=hr_sum->FindBin(fit_start); irow<=hr_sum->FindBin(fit_stop); irow++)
      {
        avg_1_p = avg_1_p + hcov_1_p->GetBinContent(irow);
	avg_1_m = avg_1_m + hcov_1_m->GetBinContent(irow);
	avg_13_p = avg_13_p + hcov_13_p->GetBinContent(irow);
	avg_13_m = avg_13_m + hcov_13_m->GetBinContent(irow);
	avg_14_p = avg_14_p + hcov_14_p->GetBinContent(irow);
	avg_14_m = avg_14_m + hcov_14_m->GetBinContent(irow);
	avg_15_p = avg_15_p + hcov_15_p->GetBinContent(irow);
	avg_15_m = avg_15_m + hcov_15_m->GetBinContent(irow);
	avg_27_p = avg_27_p + hcov_27_p->GetBinContent(irow);
	avg_27_m = avg_27_m + hcov_27_m->GetBinContent(irow);
        avg_28_p = avg_28_p + hcov_28_p->GetBinContent(irow);
	avg_28_m = avg_28_m + hcov_28_m->GetBinContent(irow);
        avg_29_p = avg_29_p + hcov_29_p->GetBinContent(irow);
	avg_29_m = avg_29_m + hcov_29_m->GetBinContent(irow);

	bincount++;

      }

   cout<<"diag+1 "<<avg_1_p/bincount<<endl;
   cout<<"diag-1 "<<avg_1_m/bincount<<endl;
   cout<<"diag+13 "<<avg_13_p/bincount<<endl;
   cout<<"diag-13 "<<avg_13_m/bincount<<endl;
   cout<<"diag+14 "<<avg_14_p/bincount<<endl;
   cout<<"diag-14 "<<avg_14_m/bincount<<endl;
   cout<<"diag+15 "<<avg_15_p/bincount<<endl;
   cout<<"diag-15 "<<avg_15_m/bincount<<endl;
   cout<<"diag+27 "<<avg_27_p/bincount<<endl;
   cout<<"diag-27 "<<avg_27_m/bincount<<endl;
   cout<<"diag+28 "<<avg_28_p/bincount<<endl;
   cout<<"diag-28 "<<avg_28_m/bincount<<endl;
   cout<<"diag+29 "<<avg_29_p/bincount<<endl;
   cout<<"diag-29 "<<avg_29_m/bincount<<endl;
   ...
   hcov_1_p->SetLineWidth(3);
   hcov_1_m->SetLineWidth(3);
 hcov_13_m->SetLineWidth(3);
 hcov_13_p->SetLineWidth(3);
 hcov_14_p->SetLineWidth(3);
 hcov_14_m->SetLineWidth(3);
 hcov_15_m->SetLineWidth(3);
 hcov_15_p->SetLineWidth(3);
 hcov_27_p->SetLineWidth(3);
 hcov_27_m->SetLineWidth(3);
 hcov_28_m->SetLineWidth(3);
 hcov_28_p->SetLineWidth(3);
 hcov_29_p->SetLineWidth(3);
 hcov_29_m->SetLineWidth(3);
...
hcov_29_p->Draw("same");
 ...
 //hcov->Scale(0.9);
   ...
   foutput=new TFile("run2H_NE_scalingtest_1009.root","new");
   hr_sum->Write();
   hcov->Write();
   foutput->Close();
