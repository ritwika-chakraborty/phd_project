...
TFile *_file[25];
TDirectory *dir[25];
TH1D *h[25],*qHist_1D[25],*h_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno, *hcalo, *hfit, *hr[25], *h_res, *hr_sum, *hr_temp, *hr_temp2;
...
TH1D* construct_rhist_copy(TH1D *h_sum)
{
 ...
 if(usesetbins)
  ...
  hcalo->Rebin(4);//This brings the bin width of the calo summed histograms to 75 ns
  hcalo->Scale(0.25);
...
void ratio_2hist_FR_cov_fullfit()
{
  ...
  // hsum1=(TH1D*)qHist_1D[1]->Clone();
   ...
   cout<<h_sum->GetBinWidth(1)<<" "<<h_sum->GetNbinsX()<<" "<<h_sum->GetBinLowEdge(1)<<" "<<(h_sum->GetBinLowEdge(h_sum->GetNbinsX())+h_sum->GetBinWidth(1))<<" "<<endl;
     ...
     fit_func19->SetNpx(1000000);
        ...
        hr_temp = construct_rhist_copy(h_sum);

	hr_temp2=(TH1D*)hr_temp->Clone();
	hr_sum=(TH1D*)hr_temp->Clone();

	hr_temp2->Reset();
	hr_sum->Reset();

	for(int ibin=1;ibin<=hr_temp2->GetNbinsX();ibin++)
	  {
	    hr_temp2->SetBinContent(ibin,hr_temp->GetBinContent(ibin+1));
	    hr_temp2->SetBinError(ibin,hr_temp->GetBinError(ibin+1));
	    
	  }

	for(int ibin=1;ibin<=hr_sum->GetNbinsX();ibin++)
	  {
	    hr_sum->SetBinContent(ibin,0.5*(hr_temp->GetBinContent(ibin)+hr_temp2->GetBinContent(ibin)));
	    hr_sum->SetBinError(ibin,0.5*(hr_temp->GetBinError(ibin)+hr_temp2->GetBinError(ibin)));
	  }
