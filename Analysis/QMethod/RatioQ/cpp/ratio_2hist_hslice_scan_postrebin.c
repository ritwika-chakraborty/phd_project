...
TFile *_file[25];
TDirectory *dir[25];
TH1D *h[25],*qHist_1D[25],*h_sum, *h0_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno, *hcalo, *hfit, *hr[25], *h_res[25];
TCanvas *c,*c1, *c2,*c3;
TGraphErrors *gr1;
TH1 *hfft[25];
...
TH1D* construct_rhist_copy(TH1D *h_sum)
{
   ...
   cout<<"Check "<<h_sum->GetBinWidth(100)<<endl;
   cout<<"Check "<<h0_sum->GetBinWidth(100)<<endl;
...
void ratio_2hist_hslice_scan_postrebin()
{
  ...
  h0_sum= new TH1D("calo histogram sum 0", "h0_sum", h[1]->GetNbinsX(), 100001, 352001);
  h0_sum->Sumw2(kTRUE);
  for(Int_t i=1; i<=6; i++)
    {
      h0_sum->Add(h[i],1); 
   ...
   double binwidth=h0_sum->GetBinWidth(1);
   int nbins=h0_sum->GetNbinsX();
   double binlowedge=h0_sum->GetBinLowEdge(1);
   double binhighedge=h0_sum->GetBinLowEdge(nbins)+binwidth;
   ...
   cout<<h0_sum->GetBinWidth(1)<<" "<<h0_sum->GetNbinsX()<<" "<<h0_sum->GetBinLowEdge(1)<<" "<<(h0_sum->GetBinLowEdge(h0_sum->GetNbinsX())+h0_sum->GetBinWidth(1))<<" "<<endl;
  ...
  hlm->SetName("hlm");
	...
	hr[i]->GetXaxis()->SetTitle("time [ns]");
        ...
        fit_func7->SetParameter(3, 0.1);
        fit_func7->SetParameter(4, 75000);
        fit_func7->SetParameter(5, 0.01393);
        fit_func7->SetParameter(6, -2.6);
       ...
       fit_func11->SetParameter(8, 200000);
       ...
       fit_func19->SetParameter(17, 0.004);
       fit_func19->SetParameter(18, 3.48);
