...
TFile *_file[52];
TDirectory *dir[52];
TH1D *h[50],*qHist_1D[52],*h_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno, *hcalo, *hfit, *hr[52], *h_res[52];
  ...
  for(Int_t i=1; i<=24; i++)
   ...
   cout<<h_sum->GetBinWidth(1)<<" "<<h_sum->GetNbinsX()<<" "<<h_sum->GetBinLowEdge(1)<<" "<<(h_sum->GetBinLowEdge(h_sum->GetNbinsX())+h_sum->GetBinWidth(1))<<" "<<endl;
	...
	hr[i]->GetXaxis()->SetTitle("time [ns]");
       ...
       fit_func11->SetParameter(7, 0.002);
       fit_func11->SetParameter(8, 30000);
       fit_func11->SetParameter(9, 0.0140);
