...
TH1D* construct_rhist_copy(TH1D *h_sum, TH1D *ho_sum)
{
 ...
 if(useerrorbars)
	...
	t9=-((4*(2*et1 + et2 + et3))/pow((4*et1 + 2*et2 + 2*et3 + et4 + et5 + 2*et6 + 2*et7 + et8 + et9),2))*det9;
	 ...
	 //cout<<"ibin "<<ibin<<"errorbar "<<sqrt( (t1*t1) + (t2*t2) + (t3*t3) + (t4*t4) + (t5*t5) + (t6*t6) + (t7*t7) + (t8*t8) + (t9*t9) )<<endl;
       }
       else
       {
...
void ratio_FR_cov_starttime()
{
  ...
  for(Int_t i=1; i<=24; i++)
   ...
   cout<<h_sum->GetBinWidth(1)<<" "<<h_sum->GetNbinsX()<<" "<<h_sum->GetBinLowEdge(1)<<" "<<(h_sum->GetBinLowEdge(h_sum->GetNbinsX())+h_sum->GetBinWidth(1))<<" "<<endl;
	...
	hcomp_sum=fast_rotation_smoothing(h_sum);
	...
	cov2.Clear();
     ...
     /*   foutput=new TFile("4h_copy_ratio_FR_starttime_cov.root","new");
   hr_sum->Write();
   for(int i=1;i<=71;i++)
     {
      hcov[i]->Write();
