...
void run2c_dqc_hslice_ratio()
 ...
 TDirectoryFile *dir[25];
  ...
  _fileh[1]=TFile::Open("run2c_dqc_thresh_300.root");
  _fileh[1]->GetObject("QFillByFillAnalyzer",dirh[1]);
  dirh[1]->GetObject("qHist1D_sig_hslice_0",qHist_hslice_1D[1]);
  hs[1]=(TH1D*)qHist_hslice_1D[1]->Clone();

  _fileh[2]=TFile::Open("run2c_dqc_thresh_300.root");
  _fileh[2]->GetObject("QFillByFillAnalyzer",dirh[2]);
  dirh[2]->GetObject("qHist1D_sig_hslice_1",qHist_hslice_1D[2]);
  hs[2]=(TH1D*)qHist_hslice_1D[2]->Clone();

   _fileh[3]=TFile::Open("run2c_dqc_thresh_300.root");
  _fileh[3]->GetObject("QFillByFillAnalyzer",dirh[3]);
  dirh[3]->GetObject("qHist1D_sig_hslice_2",qHist_hslice_1D[3]);
  hs[3]=(TH1D*)qHist_hslice_1D[3]->Clone();

   _fileh[4]=TFile::Open("run2c_dqc_thresh_300.root");
  _fileh[4]->GetObject("QFillByFillAnalyzer",dirh[4]);
  dirh[4]->GetObject("qHist1D_sig_hslice_3",qHist_hslice_1D[4]);
  hs[4]=(TH1D*)qHist_hslice_1D[4]->Clone();

   _fileh[5]=TFile::Open("run2c_dqc_thresh_300.root");
  _fileh[5]->GetObject("QFillByFillAnalyzer",dirh[5]);
  dirh[5]->GetObject("qHist1D_sig_hslice_4",qHist_hslice_1D[5]);
  hs[5]=(TH1D*)qHist_hslice_1D[5]->Clone();

   _fileh[6]=TFile::Open("run2c_dqc_thresh_300.root");
  _fileh[6]->GetObject("QFillByFillAnalyzer",dirh[6]);
 ...
 cout<<sum<<" "<<endl;
  ...
  h_sum->Sumw2(kTRUE);
   ...
   c2->Divide(1,6);
    ...
    VD_func= new TF1("fVD", fVD,  110000,  350000, 2);
    VD_func->SetParNames("A", "tau_A");
	VD_func->SetNpx(1000000);
	VD_func->SetParameters(1500, 9000);
   fit_start=110000;
     fit_stop=325600;
   //   fit_stop=200000;
   ...
   c3->cd(2);
     ...
     tau_A[2]=VD_func->GetParameter(1);
	   ...
	   // VD_func->Draw("same");
	   	...
	   	VD_func->SetParameters(20000000, 5000);
        hs[3]->Rebin(30);
   hs[3]->Scale(0.0334);
