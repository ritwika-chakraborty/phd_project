...
char root_file_name[128] = "gain_systematics_scan.root";
...
TFile *_file[50];
TDirectory *dir[50];
TH1D *h[50],*qHist_1D[50],*h_sum, *h1, *h2, *hp, *hm, *h_ratio, *h_num, *h_deno, *hcalo, *hfit, *hr[50], *h_res, *hr_sum, *hcov_1_p, *hcov_1_m,*hcov_13_p,*hcov_13_m,*hcov_14_p, *hcov_14_m, *hcov_15_p, *hcov_15_m, *hcov_27_p, *hcov_27_m,*hcov_28_p, *hcov_28_m, *hcov_29_p, *hcov_29_m, *hr_noshift, *hr_shift, *hcomp[50],*hcomp_sum,*htemp,*hrm1[50],*hrm2[50],*hrm3[50],*hrm4[50];
...
Int_t m=0;
Double_t blindR[50],dblindR[50],n[50];
Int_t countfcn=0;
...
void ratio_FR_cov_construct_perIFGmult()
{
    ...
    for(int i=1;i<=40;i++)
    {
     h[i]= (TH1D*)(_file[1]->FindObjectAny(TString::Format("hsig3a_life_mult_%d", i)));
    }
  ...
  */
  ...
  h_sum= new TH1D("calo_histogram_sum", "h_sum", h[1]->GetNbinsX(), h[1]->GetBinLowEdge(1), h[1]->GetBinLowEdge(h[1]->GetNbinsX())+h[1]->GetBinWidth(1));
  h_sum->Sumw2(kTRUE);
  for(Int_t i=1; i<=40; i++)
   ...
   cout<<h_sum->GetBinWidth(1)<<" "<<h_sum->GetNbinsX()<<" "<<h_sum->GetBinLowEdge(1)<<" "<<(h_sum->GetBinLowEdge(h_sum->GetNbinsX())+h_sum->GetBinWidth(1))<<" "<<endl;
	...
	//	return; 
    foutput=new TFile("run3a_IFG_timescan.root","new");
   for(int i=1;i<=40;i++)
     {
      hr[i]->Write();
      hcov[i]->Write();
