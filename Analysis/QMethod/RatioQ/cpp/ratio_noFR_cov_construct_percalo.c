...
char root_file_name[128] = "run3N_part3_partial.root";
...
TArrayD ratio_cov_matrix(TH1D *hr_sum, TH1D *h_sum){
...
void ratio_noFR_cov_construct_percalo()
{
  ...
  */
  ...
  for(Int_t i=1; i<=24; i++)
   ...
   //   h_sum->Scale(0.25);
	...
	for(int i=1;i<=24;i++)
	  {
	   ...
	   htemp = construct_rhist_copy(hcomp[i]);
	   ...
	   // cov2.SetTol(1.e-23);
           ...
           data2=ratio_cov_matrix(hr[i],h[i]);
    ...
    foutput=new TFile("run3N_part3_4hcopy_ratio_noFR_percalo_cov_mat.root","new");
   for(int i=1;i<=24;i++)
     {
      hr[i]->Write();
      hcov[i]->Write();
