...
Double_t fprec23(Double_t *x, Double_t *par)
{
    ...
    double asym_pr= par[19];

    double tau_pr = par[20];

    double omega_pr = par[21];

    double phi_pr = par[22];
...
Double_t fprec27(Double_t *x, Double_t *par)
{
    ...
    double A2_vbo = par[23];

    double phi2_vbo = par[24];

    double A3_vbo = par[25];

    double phi3_vbo = par[26];
    ...
    double Avbob=(1+ A2_vbo*exp(-(time - T_a/2)/tau_vw)*cos(omega_vw*(time - T_a/2) + phi2_vbo));
    ...
    double phivbob=(A3_vbo*exp(-(time - T_a/2)/tau_vw)*cos(omega_vw*(time - T_a/2) + phi3_vbo));
    ...
    double fb=(1+ asym*Acbob*Avbob*cos(omega_a*(time - T_a/2) + phi + phicbob + phivbob));
...
void ratio_hslice_scan()
{
  ...
  hlm->SetName("hlm");
     ...
     fit_func27->SetParName(3,"A_vw");
     fit_func27->SetParName(4,"Tau_vw");
     fit_func27->SetParName(5,"omega_vw");
     fit_func27->SetParName(6,"phi_vw");
     fit_func27->SetParName(7,"A_vbo");
     fit_func27->SetParName(8,"Tau_vbo");
     fit_func27->SetParName(9,"omega_vbo");
     fit_func27->SetParName(10,"phi_vbo");
     fit_func27->SetParName(11,"A_cbo");
     fit_func27->SetParName(12,"Tau_cbo");
     fit_func27->SetParName(13,"omega_cbo");
     fit_func27->SetParName(14,"phi_cbo");
     fit_func27->SetParName(15,"A2_cbo");
     fit_func27->SetParName(16,"phi2_cbo");
     fit_func27->SetParName(17,"A3_cbo");
     fit_func27->SetParName(18,"phi3_cbo");
     fit_func27->SetParName(19,"A_pr");
     fit_func27->SetParName(20,"Tau_pr");
     fit_func27->SetParName(21,"omega_pr");
     fit_func27->SetParName(22,"phi_pr");
     fit_func27->SetParName(23,"A2_vbo");
     fit_func27->SetParName(24,"phi2_vbo");
     fit_func27->SetParName(25,"A3_vbo");
     fit_func27->SetParName(26,"phi3_vbo");
	...
	hr[i]->GetXaxis()->SetTitle("time [ns]");
       ...
       */
       ...
       fit_func15->SetParameter(11, 0.006);
       fit_func15->SetParameter(12, 170000);
       fit_func15->SetParameter(13, 0.00234);
       fit_func15->SetParameter(14, 0.56);
       ...
       fit_func19->SetParameter(16, 1.6);
       ...
       fit_func23->SetParameter(20, 70000);
       fit_func23->SetParameter(21, 0.0107);
       ...
       fit_func27->SetParameter(12, fit_func23->GetParameter(12));
       fit_func27->SetParameter(13, fit_func23->GetParameter(13));
       ...
       fit_func27->SetParameter(23,0.003);
       fit_func27->SetParameter(24,3.2);
       fit_func27->SetParameter(25,0.01);
       fit_func27->SetParameter(26,0.1);


      
       hr[i]->Fit("fprec27","RE","",fit_start,fit_stop);


     
       
        blindR[m]=fit_func23->GetParameter(2);
       // blindR[m]=fit_func27->GetChisquare()/fit_func23->GetNDF();
	...
	for (int ibin = ((fit_start + 1- hr[i]->GetBinLowEdge(1))/hr[i]->GetBinWidth(1))+1; ibin <= ((fit_stop + 1 - hr[i]->GetBinLowEdge(1))/hr[i]->GetBinWidth(1))+1; ++ibin)
        {
          ...
          double res =  (hr[i]->GetBinContent(ibin)- fit_func27->Eval( hr[i]->GetBinCenter(ibin) ) );
