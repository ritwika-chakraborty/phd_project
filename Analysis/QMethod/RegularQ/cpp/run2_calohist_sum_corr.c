...
TH1D *qHist_1D[25], *h[25], *h_sum, *h_diff, *hcalo;
Double_t fit_start, fit_stop;
//TH1 *hm;
Int_t m;

void run2_calohist_sum_corr()
 ...
 cout<<sum<<" "<<endl;
  ...
  h_sum= new TH1D("calo histogram sum", "h_sum", h[12]->GetNbinsX(), 100001, 352001);
  h_sum->Sumw2(kTRUE);
   ...
   cout<<"Enter calo number"<<endl;
   cin>>m;
   hcalo= new TH1D("hcalo","hcalo", h[m]->GetNbinsX(), 100001, 352001);
   for(Int_t j=1;j<=h[m]->GetNbinsX();j++)
     {
       hcalo->SetBinContent(j, h[m]->GetBinContent(j));
       hcalo->SetBinError(j,h[m]->GetBinError(j));
     }

   
   hcalo->Rebin(4);

   h_diff= new TH1D("difference", "h_diff", h_sum->GetNbinsX(), 100001, 352001);

   for(Int_t j=1; j<=h_sum->GetNbinsX()-20; j++)
     {
       if(j>=100)
	 h_diff->SetBinContent(j,(h_sum0->GetBinContent(j)-h_sum->GetBinContent(j))/h_sum0->GetBinContent(j));

       else
	 h_diff->SetBinContent(j,0);
     }
