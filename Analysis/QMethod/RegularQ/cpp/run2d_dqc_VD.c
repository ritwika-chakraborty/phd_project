...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    double tau_VD = par[19];
    
    double time = x[0];
    
    double omega_a = 1.e-3 * getBlinded.paramToFreq(R);

    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo)))*(1 + A_VD*exp(-time/tau_VD));
...
void run2d_dqc_VD()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  30000,  309000, 20);
  ...
  // );
     ...
     fit_func->SetParName(18, "A_VD");
     fit_func->SetParName(19, "tau_VD");
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
   fit_func->SetParameters(7808060000, 64439.4, 0.234041, 0, 2.22, 0.00261778, 238510, 0.00234020, 3.57360);
   //, 0.002, 0.2);
   //, 1000000, 10000, 0.01, 0);
     ...
     fit_func->SetParameter(18, -0.01);
     fit_func->SetParameter(19, 4500);
     ...
     //fit_func->FixParameter(13,0);
           ...
           h_sum->GetYaxis()->SetRangeUser(-100., 8000000000.);
