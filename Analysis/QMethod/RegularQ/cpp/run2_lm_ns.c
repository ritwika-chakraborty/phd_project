...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3)));
...
void run2_lm_ns()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  30000,  309000, 14);
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
   fit_func->SetParameters(22369000000, 64434.4, 0.23404, 0, 2.22, 0.00260749, 239213, 0.00234018, 3.57497);
   //, 0.002, 0.2);
