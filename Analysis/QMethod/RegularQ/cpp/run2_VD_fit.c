...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    double vertical_drift = par[22];

    double vertical_drift_osc = par[23];
    
    double time = x[0];
    
    double omega_a = rawBinToNs * 1.e-3 * getBlinded.paramToFreq(R);

    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3*1.25)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo))) * (1-A_vbo2 * exp(-time/Tau_vbo2) * (cos(omega_vbo2*time+phi_vbo2))) * (1.0 + par[22] * sin(par[23]*time)* ( -2.0e-3 + 6.83011e-01*exp(-time/6.69135e+03) + 1.24584e-03*exp(-time/4.59201e+07)));
...
void run2_VD_fit()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  114000,  330000, 24);
  ...
  // );
     ...
     fit_func->SetParName(22, "VD_amp");
     fit_func->SetParName(23, "VD_osc");
...
fit_func->SetParameters(170586000000, 51525, 0.234, 0, 2.162, 0.0040, 200874, 0.002925, 3.14/4);
   ...
   //, 1000000, 10000, 0.01, 0);
     ...
     */
      ...
      fit_func->SetParameter(22, 0.1);
      fit_func->SetParameter(23, 0.0000482);
      //  fit_func->FixParameter(22,0);
      fit_func->FixParameter(23, 0.0000482);
  ...
  // TF1 *f1 = new TF1("f1","  (1.0 + [0] *sin([1]*x)* ( -2.0e-3 + 6.83011e-01*exp(-x/6.69135e+03) + 1.24584e-03*exp(-x/4.59201e+07)))", fit_start, fit_stop);
  //  TF1 *f1 = new TF1("f1","  ( [0] *sin([1]*x)* (  1*exp(-x/100000)+1*exp(-x/1000) ))", fit_start, fit_stop);
  /*  TF1 *f1 = new TF1("f1","  ( [0] *sin([1]*x)* (  -1*exp(-x/6.69135e+03)+1*exp(-x/5e+05) ))", fit_start, fit_stop);
  f1->SetParameter(0, 1);
  f1->SetParameter(1, 0.000048);
  // h_res->Fit("f1","R","", fit_start, fit_stop);
     f1->Draw("same");
  */
  c2->cd(3);
     hm = h_res->FFT(hm, "MAG");
