...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    double Tau_po = par[19];

    double omega_po = par[20];

    double phi_po = par[21];
    
    double time = x[0];
    
    double omega_a = 1.e-3 * getBlinded.paramToFreq(R);

    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo))) * (1-A_po * exp(-time/Tau_po) * (cos(omega_po*time+phi_po)));
...
void run2c_dqc_PO()
 ...
 c2->cd(1);
  ...
  // );
     ...
     fit_func->SetParName(18, "A_po");
     fit_func->SetParName(19, "Tau_po");
     fit_func->SetParName(20, "omega_po");
     fit_func->SetParName(21, "phi_po");
 ...
 gStyle->SetOptFit(1111);
   ...
   fit_func->SetParameters(1351557083, 64439.4, 0.234041, 0, 2.22, 0.00261778, 238510, 0.00234020, 3.57360);
     ...
     fit_func->SetParameter(18, 0.001);
     fit_func->SetParameter(19, 1000);
     fit_func->SetParameter(20, 0.00177);
