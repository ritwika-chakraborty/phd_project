...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo)))  * (1.0 + par[18] * (sin(par[19]*time))* ( -2.0e-3 + 6.83011e-01*exp(-time/6.69135e+03) + 1.24584e-03*exp(-time/4.59201e+07)));
...
void run2_VD_ns()
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
   fit_func->SetParameters(22369000000, 64434.4, 0.23404, 0, 2.22, 0.00260749, 238514, 0.00234018, 3.57497);
   //, 0.002, 0.2);
   //, 1000000, 10000, 0.01, 0);
   
     fit_func->SetParameter(9,0.00048);
     fit_func->SetParameter(10,0);
     fit_func->SetParameter(11, -0.000313);
     fit_func->SetParameter(12,0);
     fit_func->SetParameter(13,0);

     fit_func->SetParameter(14, 0.04);
     fit_func->SetParameter(15, 15695.9*1.25);
     fit_func->SetParameter(16, 0.15344);
