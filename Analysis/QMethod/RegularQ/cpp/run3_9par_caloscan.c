...
char root_file_name[128]="run3K_NE.root";

TCanvas *c1;
//TF1 *fit_func, *fit_func1, *cbo_func;
TH1D *qHist_1D[25], *qHist_2[25], *h[25], *hg[25], *h_sum, *h_sum2, *h_err, *h_err2, *hcalo, *herrorbar;
//TH1 *hm;
TH1F *hlm, h0;
TCanvas *c2, *c3, *c4, *c5;
TF1 *fit_func;
...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    return norm * (1 + A_cbo*exp(-time/tau_cbo)*cos(omega_cbo*time + phi_cbo)) * exp(-time/life) * (1 + asym*cos(omega_a*time + phi));
    //  return norm * exp(-time/life) * (1 + asym*cos(omega_a*time + phi));
}

void run3_9par_caloscan()
 ...
 c2=new TCanvas("c2","5 parameter wiggle_fit");
  ...
  fit_func= new TF1("fprec", fprec,  30000,  309000, 9);
 ...
 fit_func->SetNpx(1000000);
  ...
  gStyle->SetOptFit(1111);
      ...
      for(int icalo=1;icalo<=24;icalo++)
	{
	  ...
	  fit_func->SetParameters(370000000, 51540*1.25, 0.22, 0.0, 2.23, 0.007,230000,0.002329,4.5);
	  //fit_func->FixParameter(7,0.002329);
   ...
   //hm[1]->Draw();
	...
	herrorbar=(TH1D*)h[1]->Clone();
	herrorbar->Reset();

	for(int i=h[23]->FindBin(fit_start);i<=h[23]->FindBin(fit_stop);i++)
	  {
	    herrorbar->SetBinContent(i,h[23]->GetBinError(i)/h[23]->GetBinContent(i));
	  }
	*/ 
 }
