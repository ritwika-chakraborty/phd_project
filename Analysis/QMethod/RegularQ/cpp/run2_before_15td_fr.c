...
void chi2(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t inf)
{
 ...
 fit_func= new TF1("fprec", fprec,  114000,  290000, 11);
 ...
 //  fit_func->SetParameters(1580000000/2.5, 51568, 0.2, 135.54, 3.14/2, 0.01, 128325, 0.0029, 3.14/2);
  fit_func->SetParameters(158000000/9.9, 51568, 0.2, 135.54, 3.14/2, 0.01, 128325, 0.0029, 1.57, 0.0, 1.5);
  ...
  //  h[1]->Rebin(4);
   ...
   h1[1]= new TH1D("180 degree shifted histogram", "h1[1]", h[1]->GetNbinsX(), 100001, 292481);
   h1[0]= new TH1D("-180 degree shifted histogram", "h1[0]", h[1]->GetNbinsX(), 100001, 292481);
   hcomp= new TH1D("superposed histogram", "hcomp", h[1]->GetNbinsX(), 100001, 292481);   
   ...
   c4->cd(1);
    ...
    hcomp->GetYaxis()->SetRangeUser(-100., 3000000.);
 ...
 // h_res->Fit("cbo","R","",120000,280000);
 // cbo_func->Draw("same");
 
 c4->cd(2);
 h_res= new TH1D("residual histogram", "h_res", hcomp->GetNbinsX(), 100001, 292481);
