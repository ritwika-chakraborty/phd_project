...
//char root_file_name[128] = "run3N_thresh300_nofbfDQC_old.root";
//char root_file_name[128] = "run3N_thresh300_nofbfDQC_wndw_8_new.root";
char root_file_name[128]="run3N_thresh300_nofbfDQC_wndw_8_new_2.root";
TCanvas *c1;
...
void run3O_5par_vscan()
 ...
 TDirectoryFile *dir[25];
  ...
  _file[1]=TFile::Open(root_file_name);
   ...
   for(int i=1;i<=9;i++)
    {
     h[i]= (TH1D*)(_file[1]->FindObjectAny(TString::Format("qHist1D_sig_vslice_%d", i-1)));
    }
 ...
 fit_func->SetNpx(1000000);
  ...
  gStyle->SetOptFit(1111);
      ...
      //     return;
      for(int icalo=1;icalo<=9;icalo++)
	{
          cout<<"calo "<<icalo<<endl;
	  if(icalo==1){fit_func->SetParameters(1700000000/0.72, 51540*1.25, 0.2, 0.0, 3.14/2);}
	    if(icalo==2){fit_func->SetParameters(17000000000/6, 51540*1.25, 0.2, 0.0, 3.14/2);}
	      if(icalo==3){fit_func->SetParameters(17000000000/5.2, 51540*1.25, 0.2, 0.0, 3.14/2);}
	        if(icalo==4){fit_func->SetParameters(17000000000/4.7, 51540*1.25, 0.2, 0.0, 3.14/2);}
		  if(icalo==5){fit_func->SetParameters(17000000000/4.2, 51540*1.25, 0.2, 0.0, 3.14/2);}
		    if(icalo==6){fit_func->SetParameters(17000000000/3.7, 51540*1.25, 0.2, 0.0, 3.14/2);}
		      if(icalo==7){fit_func->SetParameters(17000000000/3.3, 51540*1.25, 0.2, 0.0, 3.14/2);}
		        if(icalo==8){fit_func->SetParameters(17000000000/2.9, 51540*1.25, 0.2, 0.0, 3.14/2);}
			  if(icalo==9){fit_func->SetParameters(17000000000/2.3, 51540*1.25, 0.2, 0.0, 3.14/2);}
