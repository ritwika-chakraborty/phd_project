...
void highstat_wigsim()

 {
  ...
  fit_func= new TF1("fprec", fprec,  0,  350000, 5);
  fit_func->SetParNames("N_0", "Tau", "A", "omega_a", "Phi");
  fit_func->SetNpx(10000000);
  fit_func->SetParameters(1000000, 51540, 0.2, 0.001798396, 0);
  fit_func->Draw();
  ...
  hwiggle4 = new TH1D("hwiggle4","hwiggle4", nbins, minT, maxT);
   ...
   for (Int_t j = 1; j <= hwiggle1->GetNbinsX(); j++){
     x = hwiggle1->GetBinCenter(j); //Taking the the bincenter value to fill wiggle
	...
	dy = poisson->GetRandom(); //generating poisson statistical fluctuation
      ...
      hwiggle1->Fill( x, y+dy );
      hwiggle1->SetBinError(j,sqrt(y+dy));
  ...
  */
  // hwiggle->Draw();
   ...
   foutput=new TFile("lowstat_wigsim_output_randomize.root","new");
   hwiggle1->Write();
   hwiggle2->Write();
   hwiggle3->Write();
   hwiggle4->Write();
