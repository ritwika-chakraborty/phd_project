...
void chi2(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t inf)
{
 ...
 //  fit_func->SetParameters(1580000000/2.7, 51568, 0.2, 135.54, 3.14/2, 0.01, 128325, 0.0029, 1.57, 0.0, 1.5);//calo 1
  ...
  //  h[1]->Rebin(4);
   ...
   h12[1]= new TH1D("180 degree shifted histogram", "h12[1]", h[12]->GetNbinsX(), 100001, 352001);
   h12[0]= new TH1D("-180 degree shifted histogram", "h12[0]", h[12]->GetNbinsX(), 100001, 352001);
   hcomp= new TH1D("superposed histogram", "hcomp", h[12]->GetNbinsX(), 100001, 352001);   
   for(Int_t k=1; k<=h12[1]->GetNbinsX(); k++)
     {
       h12[1]->SetBinContent(k,h[12]->GetBinContent(k+1));
       h12[0]->SetBinContent(k,h[12]->GetBinContent(k-1));
     }
   for(Int_t l=1; l<=hcomp->GetNbinsX(); l++)
     {
       hcomp->SetBinContent(l,(h[12]->GetBinContent(l)+h12[1]->GetBinContent(l))/2);
     }
   h12[1]->SetLineColor(kRed);
   hcomp->SetLineColor(kBlack);

   h[12]->GetYaxis()->SetRangeUser(-100., 60000000.);//rebin 0
   ...
   c4->cd(1);
     ...
     {
       hcomp->SetBinError(p,sqrt((1/4)*(h[12]->GetBinError(2*p-1))*(h[12]->GetBinError(2*p-1))+(1)*(h[12]->GetBinError(2*p))*(h[12]->GetBinError(2*p))+(1/4)*(h[12]->GetBinError(2*p+1))*(h[12]->GetBinError(2*p+1))));
