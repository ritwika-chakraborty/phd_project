...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo))) * (1-A_vbo2 * exp(-time/Tau_vbo2) * (cos(omega_vbo2*time+phi_vbo2))) * (1-A_pr * exp(-time/Tau_pr) * (cos(omega_pr*time+phi_pr)));
...
void run2c_reprocessed_ped_ring()
 ...
 for(Int_t k=1; k<=24; k++)
   {
     if(k!=8){sum=sum+h[k]->GetBinContent(500);}
   }
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  30000,  309000, 26);
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
   fit_func->SetParameters(44375000000, 64439.1, 0.2337, 0, 2.22, 0.002553, 238592, 0.0023405, 3.323);
   //, 0.002, 0.2);
   //, 1000000, 10000, 0.01, 0);
   
     fit_func->SetParameter(9,-0.00044);
     fit_func->SetParameter(10,-2.89);
     fit_func->SetParameter(11, -0.0000608);
     fit_func->SetParameter(12,-8.7);

     fit_func->SetParameter(13,0);

     fit_func->SetParameter(14, -0.003614);
     // fit_func->SetParameter(15, 19012);
     fit_func->SetParameter(15,25590);
     fit_func->SetParameter(16, 0.153509);
     fit_func->SetParameter(17, -2.796);
     ...
     fit_func->SetParameter(22, 0.01);
     fit_func->SetParameter(23,17782);
     fit_func->SetParameter(24, 0.0106);
     fit_func->SetParameter(25, 0);
