...
//char root_file_name[128] = "run2D_thresh100_noisecorrection_multiplier_10.root";
TCanvas *c1;
...
TH1D *h_res, *h_res9, *h_res13, *h_res17, *h_res21, *h_res22, *h_res26, *h_res29;
TH1 *hm, *hm9, *hm13, *hm17, *hm21, *hm22, *hm26, *hm29;
 ...
 fit_func->SetParNames("N_0", "Tau", "A", "Blind R", "Phi");
  ...
  c6->cd(3);
   ...
   //  return;
      ...
      c7 = new TCanvas("c7","22 parameter fit");
      c7->Divide(1,3);
      c7->cd(1);
      
      		h_sum->Fit(fit_func22,"R","",fit_start,fit_stop);
                h_sum->Draw();
		h_res22= new TH1D("residual histogram 22", "h_res 22", h_sum->GetNbinsX(), h_sum->GetBinLowEdge(1), (h_sum->GetBinLowEdge(h_sum->GetNbinsX())+h_sum->GetBinWidth(1)));
	...
	for (int ibin = ((fit_start + 1- h_sum->GetBinLowEdge(1))/h_sum->GetBinWidth(1))+1; ibin <= ((fit_stop + 1 - h_sum->GetBinLowEdge(1))/h_sum->GetBinWidth(1))+1; ++ibin)
     {
      ...
      double res =  (h_sum->GetBinContent(ibin)- fit_func22->Eval( h_sum->GetBinCenter(ibin) ) );
      if(h_sum->GetBinError(ibin)!=0){res=(res/h_sum->GetBinError(ibin));}
      h_res22->SetBinContent(ibin, (res)  );
     
      }

        h_res22->GetYaxis()->SetTitle("Energy [MeV]");
	h_res22->GetXaxis()->SetTitle("time [ns]");
	h_res22->GetXaxis()->SetRangeUser(fit_start,fit_stop);
	h_res22->GetXaxis()->SetLabelSize(0.04);
	h_res22->GetYaxis()->SetLabelSize(0.04);
        h_res22->Draw();

  c7->cd(3);
     hm22 = h_res22->FFT(hm22, "MAG");
   hm22->SetLineColor(kBlack);
   //  hm->SetBins(h_res->GetNbinsX(),0,h_res->GetNbinsX()/((h_res->GetBinLowEdge(h_res->GetNbinsX())+h_res->GetBinWidth(1))));
   hm22->SetBins(h_res22->GetNbinsX(), 0, 1000/h_res22->GetBinWidth(1));
   //hm->GetXaxis()->SetRangeUser(0,h_res->GetNbinsX()/((h_res->GetBinLowEdge(h_res->GetNbinsX())+h_res->GetBinWidth(1)))/2);
   hm22->GetXaxis()->SetRangeUser(0,1000/(2*h_res22->GetBinWidth(1)));
   hm22->GetXaxis()->SetTitle("Freq [MHz]");
   hm22->GetYaxis()->SetTitle("FFT Mag [arb. units]");
   hm22->GetXaxis()->SetLabelSize(0.04);
   hm22->GetYaxis()->SetLabelSize(0.04);
   hm22->Draw();     

   hm22->GetXaxis()->SetTitle("Freq [GHz]");
   hm22->GetYaxis()->SetTitle("FFT Mag [arb. units]");
   hm22->Draw();     
      ...
      fit_func26->SetParameter(23, 14760);
      fit_func26->SetParameter(24, 0.0106471);
