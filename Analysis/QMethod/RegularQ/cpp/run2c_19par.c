...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    */
    ...
    double Tau_vbo = par[10];

    double omega_vbo = par[11];

    double phi_vbo = par[12];
    
    double A_vbo2 = par[13];

    double Tau_vbo2 = par[14];

    double omega_vbo2 = par[15];

    double phi_vbo2 = par[16];
    ...
    // return norm * exp(-time/life) * (1 + asym* (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo)))*cos(omega_a*time + phi)) * (1-A1_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi1_cbo)))*(1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo)));
    return norm * exp(-time/life) * (1 + asym * cos(omega_a*time + phi)) * (1-A1_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi1_cbo)))*(1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo))) * (1-A_vbo2 * exp(-time/Tau_vbo2) * (cos(omega_vbo2*time+phi_vbo2)));
...
void run2c_19par()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  114000,  330000, 17);
  fit_func->SetParNames("N_0", "Tau", "A", "Blind R", "Phi", "A1_cbo", "Tau_cbo", "omega_cbo", "phi1_cbo");
  //, "A2_cbo", "phi2_cbo");
  //, "A_vbo", "Tau_vbo", "omega_vbo", "phi_vbo");
  // );
    fit_func->SetParName(9, "A_vbo");
     fit_func->SetParName(10, "Tau_vbo");
     fit_func->SetParName(11, "omega_vbo");
     fit_func->SetParName(12, "phi_vbo");
     fit_func->SetParName(13, "A_vbo2");
     fit_func->SetParName(14, "Tau_vbo2");
     fit_func->SetParName(15, "omega_vbo2");
     fit_func->SetParName(16, "phi_vbo2");
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
   fit_func->SetParameters(218000000000/1.9, 51550, 0.23, 0, 2.16, 0.004, 202000, 0.0029, 0.5);
   //, 0.0002, 0);
   //, 0.002, 0.2);
   //, 1000000, 10000, 0.01, 0);
   
    fit_func->SetParameter(9, 10000);
     fit_func->SetParameter(10,10000);
     fit_func->SetParameter(11,0.027);
     fit_func->SetParameter(12,0);
   
     fit_func->SetParameter(13, 10000);
     fit_func->SetParameter(14,10000);
     fit_func->SetParameter(15,0.017);
	...
	//fit_func->SetParLimits(6, 1.0e+5, 3.0e+5);
	//h_sum->Draw();
	// fit_func->Draw("same");
	 h_sum->Fit("fprec","REM","",130000,330000);
