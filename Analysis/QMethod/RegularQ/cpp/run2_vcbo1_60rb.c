...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3*1.25)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo))) ;
    //* (1-A_vbo2 * exp(-time/Tau_vbo2) * (cos(omega_vbo2*time+phi_vbo2)));
...
void run2_vcbo1_60rb()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  114000,  330000, 18);
...
fit_func->SetParameters(162652000000, 51641.3, 0.232192, 0, 2.16403, 0.00423897, 208305, 0.00292551, -1.4564);
   ...
   //, 1000000, 10000, 0.01, 0);
     ...
     /*    fit_func->SetParameter(18, 10);
     fit_func->SetParameter(19,2000);
     fit_func->SetParameter(20,0.19);
     fit_func->SetParameter(21, 0.0);
     */
     ...
     //fit_func->FixParameter(16, 0.191885);
     // fit_func->FixParameter(15, 16066.1);
