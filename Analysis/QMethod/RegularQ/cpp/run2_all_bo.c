...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo)))   * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo)));
...
void run2_all_bo()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  30000,  309000, 17);
  ...
  // );
     ...
     fit_func->SetParName(13, "A_vbo");
     fit_func->SetParName(14, "Tau_vbo");
     fit_func->SetParName(15, "omega_vbo");
     fit_func->SetParName(16, "phi_vbo");
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
   fit_func->SetParameters(22370600000, 64439.4, 0.234041, 0, 2.22, 0.00261778, 238510, 0.00234020, 3.57360);
   //, 0.002, 0.2);
   //, 1000000, 10000, 0.01, 0);
     ...
     fit_func->SetParameter(13, 0.00638136);
     fit_func->SetParameter(14, 193436);
     fit_func->SetParameter(15, 0.153506);
