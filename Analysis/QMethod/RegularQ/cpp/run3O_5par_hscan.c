...
//char root_file_name[128]="run2D_thresh300_noisecorrection_multiplier_10.root";
//char root_file_name[128] = "run3N_thresh300_nofbfDQC_wndw_8_new.root";
//char root_file_name[128] = "run2C_thresh200_reprocessed.root";
...
void run3O_5par_hscan()
 ...
 TDirectoryFile *dir[25];
  ...
  _file[1]=TFile::Open(root_file_name);
   ...
   for(int i=1;i<=6;i++)
    {
     h[i]= (TH1D*)(_file[1]->FindObjectAny(TString::Format("qHist1D_sig_hslice_%d", i-1)));
    }
 ...
 fit_func->SetNpx(1000000);
  ...
  gStyle->SetOptFit(1111);
      ...
      //     return;
      for(int icalo=1;icalo<=6;icalo++)
	{
          cout<<"calo "<<icalo<<endl;
	  if(icalo==1){fit_func->SetParameters(1700000000/0.95, 51540*1.25, 0.2, 0.0, 3.14/2);}
	    if(icalo==2){fit_func->SetParameters(17000000000/2.4, 51540*1.25, 0.2, 0.0, 3.14/2);}
	      if(icalo==3){fit_func->SetParameters(6800000000/0.63, 51540*1.25, 0.2, 0.0, 3.14/2);}
	        if(icalo==4){fit_func->SetParameters(6800000000/0.63, 51540*1.25, 0.2, 0.0, 3.14/2);}
		  if(icalo==5){fit_func->SetParameters(17000000000/2.4, 51540*1.25, 0.2, 0.0, 3.14/2);}
		    if(icalo==6){fit_func->SetParameters(1700000000/0.95, 51540*1.25, 0.2, 0.0, 3.14/2);}
