...
TH1D *qHist_1D[25], *h[25], *h_sum, *hcalo, *h_res[25], *h_multi_1[25],*h_multi_2[25],*h_multi_3[25],*h_multi_4[25],*h_multi_5[25],*h_multi_6[25],*h_multi_7[25],*h_multi_8[25],*h_multi_9[25],*h_multi_10[25],*h_multi_11[25],*h_multi_12[25],*h_multi_13[25],*h_multi_14[25],*h_multi_15[25];
//TH1 *hm;
TH1F *hlm, h0;
...
Int_t m=0;
Double_t blindR[25],dblindR[25],n[25],phase[25],dphase[25],chi2[25],Entries[25],dEntries[25];
...
void thresh_scan()
 ...
 TDirectoryFile *dir[25];
  ...
  hlm->SetName("hlm");
     ...
     for(int i=1;i<=24;i++)
      {
      h_multi_10[i]= (TH1D*)(_file[10]->FindObjectAny(TString::Format("qHist1D_sig_%d_0", i)));
      }
     ...
     for(int i=1;i<=24;i++)
      {
      h_multi_11[i]= (TH1D*)(_file[11]->FindObjectAny(TString::Format("qHist1D_sig_%d_0", i)));
      }
     ...
     for(int i=1;i<=24;i++)
      {
      h_multi_12[i]= (TH1D*)(_file[12]->FindObjectAny(TString::Format("qHist1D_sig_%d_0", i)));
      }
     ...
     for(int i=1;i<=24;i++)
      {
      h_multi_13[i]= (TH1D*)(_file[13]->FindObjectAny(TString::Format("qHist1D_sig_%d_0", i)));
      }
     ...
     for(int i=1;i<=24;i++)
      {
      h_multi_14[i]= (TH1D*)(_file[14]->FindObjectAny(TString::Format("qHist1D_sig_%d_0", i)));
      }
     ...
     for(int i=1;i<=24;i++)
      {
      h_multi_15[i]= (TH1D*)(_file[15]->FindObjectAny(TString::Format("qHist1D_sig_%d_0", i)));
      }
    ...
    h[9]->Sumw2(kTRUE);
       ...
       h[10]= new TH1D("calo histogram sum multiplier 10", "h_sum_multi_10", h_multi_10[1]->GetNbinsX(), 100001, 352001);
    h[10]->Sumw2(kTRUE);


       h[11]= new TH1D("calo histogram sum multiplier 11", "h_sum_multi_11", h_multi_11[1]->GetNbinsX(), 100001, 352001);
    h[11]->Sumw2(kTRUE);


       h[12]= new TH1D("calo histogram sum multiplier 12", "h_sum_multi_12", h_multi_12[1]->GetNbinsX(), 100001, 352001);
    h[12]->Sumw2(kTRUE);


       h[13]= new TH1D("calo histogram sum multiplier 13", "h_sum_multi_13", h_multi_13[1]->GetNbinsX(), 100001, 352001);
    h[13]->Sumw2(kTRUE);


       h[14]= new TH1D("calo histogram sum multiplier 14", "h_sum_multi_14", h_multi_14[1]->GetNbinsX(), 100001, 352001);
    h[14]->Sumw2(kTRUE);


       h[15]= new TH1D("calo histogram sum multiplier 15", "h_sum_multi_15", h_multi_15[1]->GetNbinsX(), 100001, 352001);
    h[15]->Sumw2(kTRUE);
   ...
   for(Int_t i=1; i<=24; i++)
    {
     ...
     h[10]->Add(h_multi_10[i],1);
     h[11]->Add(h_multi_11[i],1);
     h[12]->Add(h_multi_12[i],1);
     h[13]->Add(h_multi_13[i],1);
     h[14]->Add(h_multi_14[i],1);
     h[15]->Add(h_multi_15[i],1);
    }
  ...
  double binwidth=h[1]->GetBinWidth(1);
   ...
   cout<<h[6]->GetBinWidth(1)<<" "<<h[6]->GetNbinsX()<<" "<<h[6]->GetBinLowEdge(1)<<" "<<(h[6]->GetBinLowEdge(h[6]->GetNbinsX())+h[6]->GetBinWidth(1))<<" "<<endl;
 ...
 for(int i=1; i<=15; i++)
 {
  ...
  h[i]->GetYaxis()->SetRangeUser(-100., 5000000000.);
  ...
  */
      ...
      chi2[m]=fit_func22->GetChisquare()/fit_func22->GetNDF();
      dblindR[m]=fit_func22->GetParError(3);
      //dphase[m]=fit_func22->GetParError(4);
      Entries[m]=h[i]->GetEntries();
      //  Entries[m]=fit_func22->GetParameter(0);
      //dEntries[m]=fit_func22->GetParError(0);
      n[m]=(m+1)*50;
      m=m+1;
 ...
 c4->cd(1);
    ...
    gr1->SetTitle("R vs threshold");
    gr1->GetXaxis()->SetTitle("threshold [MeV]");
    ...
    //  gr1->GetXaxis()->SetRangeUser(5,11);
    ...
    gr2->SetTitle("dR vs threshold");
    gr2->GetXaxis()->SetTitle("threshold [MeV]");
    ...
    //  gr2->GetXaxis()->SetRangeUser(5,11);
    //  gr1->GetYaxis()->SetRangeUser(0.8,1.3);
    gr2->Draw();
    ...
    gr3->SetTitle("chi-squared vs threshold");
    gr3->GetXaxis()->SetTitle("chi-squared");
    gr3->GetYaxis()->SetTitle("threshold [MeV]");
    gr3->SetMarkerStyle(20);
    gr3->SetLineColor(kRed);
    gr3->RemovePoint(0);
     gr3->GetXaxis()->SetLabelSize(0.05);
    gr3->GetYaxis()->SetLabelSize(0.05);
    //   gr3->GetXaxis()->SetRangeUser(5,11);
    //  gr1->GetYaxis()->SetRangeUser(0.8,1.3);
    gr3->Draw();


    c5=new TCanvas("c5","number of entries vs threshold");
    gr4=new TGraphErrors(m,n,Entries,0,0);
    gr4->SetTitle("Entries vs threshold");
    gr4->GetXaxis()->SetTitle("threshold [MeV]");
    gr4->GetYaxis()->SetTitle("Entries");
    gr4->SetMarkerStyle(20);
    gr4->SetLineColor(kRed);
    gr4->RemovePoint(0);
    //  gr4->GetXaxis()->SetRangeUser(5,11);
