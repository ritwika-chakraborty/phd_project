...
char root_file_name[128] = "run2CDEFGH.root";
...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    /* if(x[0]<=239551.25 && x[0]>=239401.25)
      {
	TF1::RejectPoint();
	return 0;
      }
...
void run2_18par()
 ...
 // h_sum->Rebin(8);
   ...
   h_sum->Rebin(8);
     fit_func->SetParameters(111853000000/1.4, 64439.4, 0.234041, 0, 2.22, 0.00261778, 238510, 0.00234020, 3.57360);
   // fit_func->SetParameters(111853000000/(2.8), 64439.4, 0.234041, 0, 2.8, 0.00261778, 238510, 0.00234020, 3.57360);
   //, 0.002, 0.2);
   //, 1000000, 10000, 0.01, 0);
     ...
     //fit_func->FixParameter(13,0);
           ...
           h_sum->GetYaxis()->SetRangeUser(-100., 85000000000.);
