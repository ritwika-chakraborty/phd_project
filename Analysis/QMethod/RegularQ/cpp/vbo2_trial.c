...
TH1D *qHist_1D[25], *h[25], *h_sum, *hcalo, *h_sum0;
//TH1 *hm;
...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    //   double vdrift_amp = par[18];

    // double vdrift_osc = par[19];
    
    double time = x[0];
    
    double omega_a = 1.25*1.e-3 * getBlinded.paramToFreq(R);
    ...
    //   return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo))) *  (1-A_vbo2 * exp(-time/Tau_vbo2) * (cos(omega_vbo2*time+phi_vbo2)));
      //* (1.0 + par[18] * (sin(par[19]*time))* ( -2.0e-3 + 6.83011e-01*exp(-time/6.69135e+03) + 1.24584e-03*exp(-time/4.59201e+07)));
...
void vbo2_trial()
 ...
 cout<<sum<<" "<<endl;
  ...
  h_sum->Sumw2(kTRUE);
   ...
   //  h_sum= new TH1D("calo histogram sum", "h_sum", h[1]->GetNbinsX(), 0, 315000);

   double resett0 = 1.048e5;
   double resetbins = h_sum->GetNbinsX(); 
   double resetloedge = h_sum->GetBinLowEdge(1);
   double resethiedge = h_sum->GetBinLowEdge(resetbins)+h_sum->GetBinWidth(resetbins);
   //  h_sum->SetBins(resetbins,rawBinToNs*(resetloedge-resett0), rawBinToNs*(resethiedge-resett0));

   //  h_sum->SetBins(16800,-5998.75, 309001.25);
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  10000,  309000, 5);
  ...
  // );
     ...
     //  fit_func->SetParName(18, "vdrift_amp");
     //fit_func->SetParName(19, "vdrift osc");
...
fit_func->SetParameters(170577000000, 51524.2, 0.234035, 0, 2.1616, 0.00431492, 201007*1.25, 0.00292548/1.25, -1.44150);

       fit_func->SetParameter(9, 0.000946335);
     fit_func->SetParameter(10, -7.33471*1.25);
     fit_func->SetParameter(11, 0.000272795/1.25);
     fit_func->SetParameter(12, -1.12846);
   ...
   //, 1000000, 10000, 0.01, 0);
     ...
     fit_func->SetParameter(14, 4.89930);
     fit_func->SetParameter(15, 15695.9*1.25);
     fit_func->SetParameter(16,0.191885/1.25);
     fit_func->SetParameter(17, 0);

      fit_func->SetParameter(18, 0);
     fit_func->SetParameter(19, 100000*1.25);
     fit_func->SetParameter(20, 0);
     fit_func->SetParameter(21, 0);

      fit_func->FixParameter(18, 0);
  
     //  fit_func->FixParameter(20, 0.188);
	//fit_func->FixParameter(19, 76526);
     //fit_func->FixParameter(16, 0.191875);
      //fit_func->FixParameter(15, 35578.1);
      //  fit_func->FixParameter(18, 0);
      
     /*   fit_func->SetParameter(18, -0.386939);
      fit_func->SetParameter(19, 0.0000497);
      //  fit_func->FixParameter(22,0);
        fit_func->FixParameter(19, 0.0000482);
     */
  ...
  c2->cd(3);
   ...
   //  hm->SetNPX(10000);
   hm->GetYaxis()->SetRangeUser(0, 160);
