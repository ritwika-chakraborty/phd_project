...
Double_t fprec(Double_t *x, Double_t *par)
{
    ...
    */
    ...
    return norm * exp(-time/life) * (1 + asym * (1-A2_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi2_cbo))) * cos(omega_a*time + (phi + A3_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi3_cbo))))) * (1-A_cbo * exp(-time/Tau_cbo) * (cos(omega_cbo*time+phi_cbo))) * ( 1- lost_muon_amp * hlm->GetBinContent(hlm->GetXaxis()->FindBin( time * 1.0e-3*1.25)))  * (1-A_vbo * exp(-time/Tau_vbo) * (cos(omega_vbo*time+phi_vbo)))  * (1.0 + par[18] * (sin(par[19]*time))* ( -2.0e-3 + 6.83011e-01*exp(-time/6.69135e+03) + 1.24584e-03*exp(-time/4.59201e+07)));
...
void run2_VD_60rb()
 ...
 c2->cd(1);
  ...
  fit_func= new TF1("fprec", fprec,  114000,  330000, 20);
 ...
 //  c2->cd(1);
   ...
   h_sum->Rebin(2);
    ...
    fit_func->SetParameters(162652000000, 51641.3, 0.232192, 0, 2.16403, 0.00423897, 208305, 0.00292551, -1.4564);

       fit_func->SetParameter(9, -0.000782051);
     fit_func->SetParameter(10, 2.00089);
     fit_func->SetParameter(11, -0.000252617);
     fit_func->SetParameter(12, -3.95727);
     ...
     fit_func->SetParameter(14, 0.83862);
     fit_func->SetParameter(15, 17650.9);
     fit_func->SetParameter(16,0.191885);
