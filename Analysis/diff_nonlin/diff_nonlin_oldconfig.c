...
TH1D *qHist_1D[25], *h[25], *h_sum, *hcalo, *h_odd[25], *h_even[25], *h_diff[25], *h_average[25], *stat_hist[25], *hstat, *h_diff_fft[25];
//TH1 *hm;
...
char root_file_name1[128] = "run2c_dqc_thresh_300_oldcongif2.root";
//char root_file_name1[128] = "run2c_test_24689_250.root";

void diff_nonlin_oldconfig()
 ...
 TDirectoryFile *dir[25];
   ...
   _file[0]=TFile::Open(root_file_name1);
 ...
 for(Int_t j=1; j<=24; j++)
  ...
  //   cout<<"j ="<<j<<endl;
   ...
   for(Int_t j=1; j<=24; j++)
	 ...
	 // c1->cd(j);
      //  h_diff[j]->Fit("pol1","R","",150000, 350000);
       ...
       p0[m]=h_diff[j]->Integral(1240,1574)/(335*hstat->Integral());
     // dp0[m]=myfit->GetParError(0);
      // h_diff[j]->GetYaxis()->SetRangeUser(-3, 9);
       //   h_diff[j]->GetYaxis()->SetRangeUser(-120000000,40000000);
       // h_diff[j]->GetYaxis()->SetRangeUser(-3000,2000);
       //h_diff[j]->Draw();
     m=m+1;
      }
   ...
   c2=new TCanvas("c2","odd even difference parameter");   
    ...
    gr1->SetTitle("normalized difference vs calo#, 14000 subruns, Run2C with run1 config");
    gr1->GetXaxis()->SetTitle("calo #");
    gr1->GetYaxis()->SetTitle("Inetgral(odd-even)/no. of entries");
