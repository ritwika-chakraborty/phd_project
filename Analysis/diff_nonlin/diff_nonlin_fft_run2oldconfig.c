...
void diff_nonlin_fft_run2oldconfig()
  ...
  //   cout<<"j ="<<j<<endl;
   ...
   for(Int_t j=1; j<=24; j++)
	 ...
	 h_average[j]->SetBinContent(i,(h_even[j]->GetBinContent(i)+h_odd[j]->GetBinContent(i))/2);
        ...
        for(Int_t j=1; j<=24; j++)
      {
       h_diff_fft[j]= new TH1D("hdiff_fft", "h_diff_fft", 667, 180000, 230025);
       
       for(Int_t i=2480; i<=3147; i++)
       {
         h_diff_fft[j]->SetBinContent(i-2479,h[j]->GetBinContent(i));
    ...
    c2->Divide(4,6);
	 ...
	 //   hm[j]->GetXaxis()->SetRangeUser(0,1/(1.99*h_diff_fft[j]->GetBinWidth(1)));
         ...
         hm[j]->GetYaxis()->SetTitle("FFT Mag [arb. units]");
      	 ...
      	 hm[j]->GetYaxis()->SetRangeUser(0,30000000);
       	 c2->cd(j);
	 // hm[j]->Draw();
	 	 h[j]->Rebin(2);
	  h[j]->GetXaxis()->SetRangeUser(150000,151000);
	 h[j]->Draw();
