...
char root_file_name5[128] = "run2d_1000sr_thresh_300.root";
char root_file_name4[128] = "run2d_1000sr_thresh_225.root";
char root_file_name3[128] = "run2d_1000sr_thresh_150.root";
char root_file_name2[128] = "run2d_1000sr_thresh_80.root";
char root_file_name1[128] = "run2d_1000sr_thresh_10.root";


void diff_nonlin_theshscan()
  ...
  for(Int_t j=1; j<=5; j++)
    ...
    h_average[j]= new TH1D("raw trace odd-even average", "h_average", h[j]->GetNbinsX()/2, 100001, 352001);
        ...
        if(i%2==0)
	 {
	   ...
	   h_even[j]->SetBinError(i/2, h[j]->GetBinError(i));
	 }
        else
	 {
           h_odd[j]->SetBinContent((i+1)/2, h[j]->GetBinContent(i));
	   h_odd[j]->SetBinError((i+1)/2, h[j]->GetBinError(i));
	 }
        }
    } 
  //   cout<<"j ="<<j<<endl;
   ...
   for(Int_t k=1; k<=5;k++)
    ...
    gr1->SetTitle("Picket fence amplitude vs threshold ");
