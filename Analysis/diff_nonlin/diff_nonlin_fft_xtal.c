...
TH1D *qHist_1D[55], *h[55], *h_sum, *hcalo, *h_odd[55], *h_even[55], *h_diff[55], *h_average[55], *hstat, *h_diff_fft[55];
//TH1 *hm;
...
char root_file_name[128] = "run2d_dqc_26013_100sr_calo1_xtals.root";
//char root_file_name[128] = "run2d_dqc_26013_1flush_xtals.root";

void diff_nonlin_fft_xtal()
   ...
   //fourier transform of the difference histogram
	 ...
	 //   hm[j]->GetXaxis()->SetRangeUser(0,1/(1.99*h_diff_fft[j]->GetBinWidth(1)));
 	 ...
 	 hm[j]->SetLineColor(kBlack);
      	 hm[j]->GetYaxis()->SetRangeUser(0,500000);
	 c1->cd(j+1);
	 hm[j]->Draw();
